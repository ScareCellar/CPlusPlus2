...
namespace blood {
	...
	public:
		...
		bool GetButtonPressed(MouseButton button) { return !m_prevMouseButtonState[(uint8_t)button] && m_keyboardState[(uint8_t)button]; }
		bool GetButtonReleased(MouseButton button) { return m_prevMouseButtonState[(uint8_t)button] && !m_mouseButtonState[(uint8_t)button]; }
